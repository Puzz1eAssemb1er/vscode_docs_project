...
- [Руководство пользователя: управление общими папками в сервисе «Яндекс Диск»](#руководство-пользователя-управление-общими-папками-в-сервисе-яндекс-диск)
  ...
  - [Схемы процессов](#схемы-процессов)
    - [Настройка общего доступа](#настройка-общего-доступа)
    - [Удаление общей папки](#удаление-общей-папки-1)
  - [Типовые проблемы и их решенияя](#типовые-проблемы-и-их-решенияя)
    - [Что делать, если ничего не помогает](#что-делать-если-ничего-не-помогает)
...
## Типовые проблемы и их решенияя
...
1. **Проверьте статус сервиса.** Иногда проблемы связаны с временными сбоями на стороне Яндекс Диска.
2. **Попробуйте повторить действие в другом браузере** или на другом устройстве — это помогает выявить локальные проблемы кэша или расширений.
3. **Сделайте скриншоты:** кнопки, которая не нажимается, и окна с ошибкой (если есть). Это сильно ускорит помощь поддержки.
4. **Обратитесь в поддержку Яндекса**, приложив скриншоты и краткое описание шагов, которые вы уже пробовали.
...
<details>
  ...
  Проверьте, какой уровень прав ему назначен. При уровне «Только просмотр» изменения не отображаются для редактирования. Также возможна задержка синхронизации на стороне серверов Яндекса (обычно занимает 1–2 минуты).
</details>
...
<details>
  <summary><strong>❓ Почему не работает ссылка в оглавлении?</strong></summary>

  Скорее всего, изменился заголовок раздела, но якорь в оглавлении не обновился. Проверьте совпадение текста заголовка и ID якоря. На GitHub якорь формируется автоматически: пробелы заменяются на дефисы, заглавные буквы — на строчные, знаки препинания удаляются.
</details>

<details>
  <summary><strong>❓ Где найти скриншоты?</strong></summary>

  Все иллюстрации хранятся в папке `images`.
</details>

<details>
  <summary><strong>❓ Как обновить схему процесса?</strong></summary>

  Откройте блок кода с `mermaid` в `README.md` и измените нужные строки — GitHub автоматически перерисует диаграмму.
</details>